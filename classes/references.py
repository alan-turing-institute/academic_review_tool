...
def extract_references(references_list, add_work_ids = True, update_from_doi = False):
    ...
    if (type(references_list) == list) and (len(references_list) > 0) and (type(references_list[0]) == dict):
    ...
    if (type(references_list) == list) and (len(references_list) > 0) and (type(references_list[0]) == str):
